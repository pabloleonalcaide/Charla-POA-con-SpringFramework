...
public class AdminResourceFunctionalTesting {
    ...
    public void testEcho() {
        ...
        System.out.println("Response: " + response);
        assertEquals("{\"id\":666,\"token\":\"Token\",\"param\":\"Param\"}", response);
    }

    @Test
    public void testEchoRestBuilder() {
        String response = new RestBuilder<String>(URL_API).path(Uris.ADMINS).path(Uris.ECHO).path("/666").param("param", "param")
                .param("other", "other").header("token", "Token").clazz(String.class).get().build();
        System.out.println("Response: " + response);
        assertEquals("{\"id\":666,\"token\":\"Token\",\"param\":\"param\"}", response);
    }
